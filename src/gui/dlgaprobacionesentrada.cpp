...
const QString sql_cargos =
    "SELECT DISTINCT details->>'aprobación_cargo' AS cargo "
    "FROM resolutions_details WHERE details ? 'aprobación' ORDER BY cargo";

const QString sql_institucion =
    "SELECT DISTINCT details->>'tipo_institucion' AS institucion "
    "FROM resolutions_details WHERE details ? 'aprobación' ORDER BY "
    "institucion";

const QString sql_librekeys =
    "SELECT DISTINCT jsonb_object_keys(details) as keys "
    "FROM resolutions_details WHERE details ? 'aprobación' ORDER BY keys";

// con esto evitamos que dé números que son muy frecuentes en los values del
// jsonb details.
const QString sql_librevalues =
...
dlgAprobacionesEntrada::dlgAprobacionesEntrada(QWidget *parent)
    : QDialog(parent), ui(new Ui::dlgAprobacionesEntrada)
{
  ui->setupUi(this);
  ...
  connect(ui->btProvincia, SIGNAL(clicked(bool)), this,
          SLOT(anadirProvincia()));
  connect(ui->btProvinciaInstitucion, SIGNAL(clicked(bool)), this,
          SLOT(anadirProvincia()));
  connect(ui->btCasa, SIGNAL(clicked(bool)), this, SLOT(anadirCasa()));
  connect(ui->btAnadirAprobacion, SIGNAL(clicked(bool)), this,
          SLOT(anadirAprobacion()));
  connect(ui->btQuitarAprobacion, SIGNAL(clicked(bool)), this,
          SLOT(quitarAprobacion()));
  connect(ui->btOK, SIGNAL(clicked(bool)), this, SLOT(aceptarAprobaciones()));
  connect(ui->tabWidget, SIGNAL(currentChanged(int)), this,
          SLOT(tabCambiado(int)));

  cargarModelos();
}

dlgAprobacionesEntrada::~dlgAprobacionesEntrada() { delete ui; }
  ...
  if (tabSeleccionado == 0) {  // el de personas
    if (ui->txtCargo->text().isEmpty()) {
      int ret = QMessageBox::warning(
          this, "No hay texto en el tipo de aprobación",
...
void dlgAprobacionesEntrada::anadirProvincia()
{
  ...
  connect(dlgseleccionar, SIGNAL(provinciaEscogidaSignal(Provincia)), this,
          SLOT(actualizarProvincia(Provincia)));
...
void dlgAprobacionesEntrada::cerrar() { parentWidget()->close(); }
