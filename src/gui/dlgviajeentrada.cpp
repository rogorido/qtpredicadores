...
const QString sql_motivos =
    "SELECT DISTINCT jsonb_array_elements_text(details->'viaje'->'motivos') AS "
    "motivos "
    "FROM persons_details WHERE details->'viaje'->>'motivos' IS NOT NULL "
    "ORDER BY motivos";
const QString sql_motivos_muerte =
    "SELECT DISTINCT details->'viaje'->>'motivo_muerte' AS motivo "
    "FROM persons_details WHERE details->'viaje'->>'motivo_muerte' IS NOT NULL "
    "ORDER BY motivo";
const QString sql_lugares_muerte =
    "SELECT DISTINCT details->'viaje'->>'lugar_muerte' AS lugar "
...
dlgViajeEntrada::dlgViajeEntrada(QWidget *parent)
    : QWidget(parent), ui(new Ui::dlgViajeEntrada)
{
  ui->setupUi(this);
  ...
  connect(ui->btQuitarMotivo, SIGNAL(clicked(bool)), this,
          SLOT(quitarMotivo()));
...
dlgViajeEntrada::~dlgViajeEntrada() { delete ui; }
...
void dlgViajeEntrada::aceptarViaje()
{
  ...
  if (ui->wdNotas->haCambiado()) viaje.setNotas(ui->wdNotas->getNotas());
...
void dlgViajeEntrada::anadirMotivo()
{
  if (!ui->txtMotivo->text().isEmpty()) {
    ...
    QListWidgetItem *item =
        new QListWidgetItem(ui->txtMotivo->text(), ui->twMotivos);
...
void dlgViajeEntrada::cerrar() { parentWidget()->close(); }
