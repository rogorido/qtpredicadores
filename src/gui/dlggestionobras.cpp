...
const QString sql_con_reedicion =
    "SELECT DISTINCT work_id FROM works_details WHERE details @> "
    "'{\"tipo\":\"reedición\"}'";

const QString sql_estadisticas_reediciones =
    "SELECT work_id, title, autor, place_print_original, date_print, COUNT(*) "
    "AS total "
...
dlgGestionObras::dlgGestionObras(QWidget *parent)
    : QWidget(parent), ui(new Ui::dlgGestionObras)
{
  ui->setupUi(this);
  ...
  sql_gestor_estadisticas =
      new SqlFiltroGestor(sql_estadisticas_reediciones, this);
  ...
  connect(sql_gestor_general, SIGNAL(actualizadoSqlFiltroGestor(QString)), this,
          SLOT(actualizarSqlGeneral(QString)));
  connect(sql_gestor_general, SIGNAL(actualizadoSqlFiltroGestor(QString)), this,
          SLOT(actualizarSqlEstadisticas(QString)));
  connect(ui->tvObras->selectionModel(),
          SIGNAL(currentRowChanged(QModelIndex, QModelIndex)), this,
          SLOT(seleccionarObra(QModelIndex)));
  connect(ui->tvObrasReediciones->selectionModel(),
          SIGNAL(currentRowChanged(QModelIndex, QModelIndex)), this,
          SLOT(seleccionarObra(QModelIndex)));
  ...
  connect(ui->tvObras, SIGNAL(customContextMenuRequested(const QPoint &)), this,
          SLOT(menuContextual(const QPoint &)));
}
...
void dlgGestionObras::modificarObra()
{
  ...
  connect(dlgObraAModificar, SIGNAL(introducidaObra()), this,
          SLOT(actualizarModeloTrasObraActualizada()));
  window->show();
}
...
void dlgGestionObras::seleccionarObra(const QModelIndex &idx)
{
  ...
  if (ui->tabWidget->currentIndex() == 0) {
    ...
    indice =
        works_statistics_model->index(row, 0);  // cuidado aquí work_id es 0
    id = indice.data().toInt();
  }
...
void dlgGestionObras::recibirAutor(const Persona autor)
{
  ...
  QListWidgetItem *item =
      new QListWidgetItem(nuevoautor.elemento, ui->lwAutores);
  Q_UNUSED(item)
...
void dlgGestionObras::generarSQLMaterias()
{
  ...
  for (int var = 0; var < materias_escogidas.size(); ++var) {
    sql += QString("theme_id = ") +
           QString::number(materias_escogidas.at(var).id) + QString(" OR ");
  }
  // borramos el último OR
  sql.chop(4);
  sql =
      QString("work_id IN (SELECT DISTINCT work_id FROM works_themes WHERE ") +
      sql + QString(")");
...
void dlgGestionObras::generarSQLAutores()
{
  ...
  for (int var = 0; var < autores_escogidos.size(); ++var) {
    sql += QString("author_id = ") +
           QString::number(autores_escogidos.at(var).id) + QString(" OR ");
  }
  // borramos el último OR
  sql.chop(4);
  sql = QString("work_id IN (SELECT DISTINCT work_id FROM works WHERE ") + sql +
        QString(")");
...
void dlgGestionObras::mostrarMaterias(const int obra_id)
{
  ...
  for (int var = 0; var < temas.size(); ++var) {
    query.exec(
        QString("SELECT * FROM themes WHERE theme_id=%1").arg(temas.at(var)));
    query.first();
    /* añadimos un elem a la tabla */
    QListWidgetItem *item =
        new QListWidgetItem(query.value(1).toString(), ui->lwMaterias);
  }
}
...
void dlgGestionObras::on_ckSinMateria_stateChanged(int arg1)
{
  ...
  if (ui->ckSinMateria->checkState() == Qt::Checked)
    sql_gestor_general->anadirFiltro(
        "sinmaterias",
        "work_id NOT IN (SELECT DISTINCT work_id FROM works_themes)");
  else
...
void dlgGestionObras::on_btModificarObra_clicked() { modificarObra(); }
