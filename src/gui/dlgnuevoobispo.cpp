...
dlgNuevoObispo::dlgNuevoObispo(QWidget *parent)
    : QWidget(parent), ui(new Ui::dlgNuevoObispo)
{
  ui->setupUi(this);
  ...
  connect(ui->dtFechaInicio, SIGNAL(dateChanged(QDate)), this,
          SLOT(fechaInicioCambiada()));
  connect(ui->dtFechaFinal, SIGNAL(dateChanged(QDate)), this,
          SLOT(fechaFinalCambiada()));

  cargarModelos();
}

dlgNuevoObispo::~dlgNuevoObispo() { delete ui; }
...
void dlgNuevoObispo::aceptarObispo()
{
  ...
  if (!papa_seleccionado.isEmpty()) papa_id = extraerPapa(papa_seleccionado);
   ...
   */
  query.prepare(
      "INSERT INTO bishops.bishops(bishop_person_id, diocese_id, pope_id, "
      "date_nomination, circa_date_nomination, date_end, duration, "
      "end_bydeath, "
...
void dlgNuevoObispo::fechaInicioCambiada() { fecha_inicio_cambiada = true; }

void dlgNuevoObispo::fechaFinalCambiada() { fecha_final_cambiada = true; }
...
void dlgNuevoObispo::cerrar() { parentWidget()->close(); }
...
void dlgNuevoObispo::introducirJson(const int id)
{
  ...
  for (int var = 0; var < totaljson; ++var) {
    ...
    query.prepare(
        "INSERT INTO bishops_details(bishop_id, details) VALUES(:bishopid, "
        ":json)");
...
void dlgNuevoObispo::introducirNotas(const int id)
{
  ...
  if (!ui->wdNotas->haCambiado()) return;
...
void dlgNuevoObispo::cargarModelos()
{
  ...
  m_papas->setQuery(
      "SELECT pope_id, name_pope FROM general.popes ORDER BY name_pope");
