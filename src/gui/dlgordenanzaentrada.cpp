...
const QString sql_receptores =
    ...
    "ORDER BY penados";
const QString sql_objetos =
    "SELECT DISTINCT jsonb_array_elements_text(details->'objetos') AS objetos "
    "FROM resolutions_details WHERE details->>'objetos' IS NOT NULL "
    "ORDER BY objetos";

dlgOrdenanzaEntrada::dlgOrdenanzaEntrada(QWidget *parent)
    : QWidget(parent), ui(new Ui::dlgOrdenanzaEntrada)
{
  ui->setupUi(this);
  ...
  connect(ui->btEliminarObjetos, SIGNAL(clicked(bool)), this,
          SLOT(quitarObjeto()));
  connect(ui->btEliminarReceptores, SIGNAL(clicked(bool)), this,
          SLOT(quitarReceptor()));
...
dlgOrdenanzaEntrada::~dlgOrdenanzaEntrada() { delete ui; }
...
void dlgOrdenanzaEntrada::aceptar()
{
  ...
  if (ui->wdNotas->haCambiado()) ordenanza.setNota(ui->wdNotas->getNotas());
...
void dlgOrdenanzaEntrada::anadirObjeto()
{
  if (!ui->txtObjeto->text().isEmpty()) {
    ...
    QListWidgetItem *item =
        new QListWidgetItem(ui->txtObjeto->text(), ui->lwObjetos);
...
void dlgOrdenanzaEntrada::recibirPena(Pena pena) { pena_estipulada = pena; }
...
void dlgOrdenanzaEntrada::cerrar() { parentWidget()->close(); }
