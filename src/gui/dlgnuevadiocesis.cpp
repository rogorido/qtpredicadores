...
dlgNuevaDiocesis::dlgNuevaDiocesis(QWidget *parent)
    : QWidget(parent), ui(new Ui::dlgNuevaDiocesis)
{
  ui->setupUi(this);
  cargarUI();
}
...
dlgNuevaDiocesis::~dlgNuevaDiocesis() { delete ui; }
...
void dlgNuevaDiocesis::aceptarDiocesis()
{
  ...
  if (modificando) diocesis_id = diocesis_modificando;
  ...
  if (modificando) diocesis->setId(diocesis_id);
  ...
  if (!ui->txtURL->text().isEmpty() ||
    ...
    if (ui->spParroquias != 0)
      datos_concretos.insert("parishes_number",
                             QJsonValue(ui->spParroquias->value()));
    if (ui->spSuperficie != 0)
      datos_concretos.insert("area", QJsonValue(ui->spSuperficie->value()));
    if (ui->spFundacion != 0)
      datos_concretos.insert("foundation",
                             QJsonValue(ui->spFundacion->value()));

    otros_datos.insert("gcatholic", datos_concretos);
    diocesis->setOtrosDatos(otros_datos);
  }

  if (ui->wdNotas->haCambiado()) diocesis->setNota(ui->wdNotas->getNotas());

  if (!modificando) {  // estamos añadiendo una diócesis
    if (!m_diocesis->AnadirDiocesis(diocesis, true)) {
      int ret =
          QMessageBox::warning(this, "Error al introducir la diócesis",
    ...
    if (!m_diocesis->AnadirDiocesis(diocesis, false)) {
      int ret =
          QMessageBox::warning(this, "Error al actualizar la diócesis",
...
{
  dlgSeleccionarGeneral *seleccionlugar =
      new dlgSeleccionarGeneral(LUGAR, true, this);
  connect(seleccionlugar, SIGNAL(lugarEscogidoSignal(Lugar)), this,
...
void dlgNuevaDiocesis::cerrar() { parentWidget()->close(); }
...
void dlgNuevaDiocesis::cargarUI()
{
  ...
  connect(ui->txtArchidiocesis, SIGNAL(dobleclick()), this,
...
void dlgNuevaDiocesis::cargarDiocesis()
{
  ...
  Diocesis *diocesisModificada =
      m_diocesis->devolverDiocesis(diocesis_modificando);
  ...
  ui->txtMotivoDesaparicion->setText(
      diocesisModificada->getMotivoDesaparicion());
  ...
  if (!json_notas.isEmpty()) ui->wdNotas->importNota(json_notas);
  ...
  if (diocesisModificada->getSufraganea() != 0) {
    Diocesis *archidiocesis =
        m_diocesis->devolverDiocesis(diocesisModificada->getSufraganea());
