...
dlgAsistentes::dlgAsistentes(int chapter, QWidget *parent)
    : QDialog(parent), ui(new Ui::dlgAsistentes), chapterescogido(chapter)
{
  ui->setupUi(this);
  ...
  connect(ui->btAceptarCasas, SIGNAL(clicked(bool)), this,
          SLOT(aceptarCasas()));
  connect(ui->btAnadirProvincia, SIGNAL(clicked(bool)), this,
          SLOT(anadirProvincia()));
  connect(ui->btQuitarProvincia, SIGNAL(clicked(bool)), this,
  ...
  connect(ui->twProvinciasSinSeleccionar, SIGNAL(doubleClicked(QModelIndex)),
          this, SLOT(anadirProvincia()));

  desmarcarTodasProvincias();
  cargarModelos();
}

dlgAsistentes::~dlgAsistentes() { delete ui; }
...
void dlgAsistentes::aceptar()
{
  ...
  for (int i = 0; i < provinciasescogidas.size(); ++i) {
    ...
    QSqlQuery query;
    query.prepare(
        "INSERT INTO chapters.chapters_details(chapter_id, details) "
        "VALUES(:id, :detalles)");
...
{
  QModelIndex idx = provincias->index(
      ui->twProvinciasSinSeleccionar->currentIndex().row(), 0);
  QModelIndex idxnombre = provincias->index(
      ui->twProvinciasSinSeleccionar->currentIndex().row(), 1);

  if (!idx.isValid() || !idxnombre.isValid()) return;
  ...
  QListWidgetItem *item =
      new QListWidgetItem(nuevo.elemento, ui->twProvinciasSeleccionadas);
}
...
void dlgAsistentes::quitarProvincia()
{
  ...
  ui->twProvinciasSeleccionadas->takeItem(
      ui->twProvinciasSeleccionadas->currentRow());
...
void dlgAsistentes::cargarModelos()
{
  ...
  ui->twProvinciasSinSeleccionar->setSelectionBehavior(
      QAbstractItemView::SelectRows);
  ui->twProvinciasSinSeleccionar->setSelectionMode(
      QAbstractItemView::SingleSelection);

  ui->twProvinciasSeleccionadas->setSelectionMode(
      QAbstractItemView::SingleSelection);
}
