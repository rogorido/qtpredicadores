...
const QString sql_editores =
    "SELECT DISTINCT details->>'editor' FROM works_details ORDER BY 1";
const QString sql_lenguas =
    "SELECT DISTINCT details->>'language' FROM works_details ORDER BY 1";
const QString sql_formats =
    "SELECT DISTINCT details->>'format' FROM works_details ORDER BY 1";

dlgNuevaReedicion::dlgNuevaReedicion(QWidget *parent)
    : QWidget(parent), ui(new Ui::dlgNuevaReedicion)
{
  ui->setupUi(this);
...
dlgNuevaReedicion::~dlgNuevaReedicion() { delete ui; }
...
void dlgNuevaReedicion::aceptadaReedicion()
{
  ...
  if (elem_lugar.id != 0) datos.insert("lugar", QJsonValue(elem_lugar.id));
...
void dlgNuevaReedicion::on_pbOK_clicked() { aceptadaReedicion(); }

void dlgNuevaReedicion::on_pbCancelar_clicked() { parentWidget()->close(); }
