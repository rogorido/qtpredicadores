...
dlgSeleccionarGeneral::dlgSeleccionarGeneral(tiposeleccionar valor, bool cerrar,
                                             QWidget *parent)
    : QWidget(parent),
      ui(new Ui::dlgSeleccionarGeneral),
      tipo_seleccionado(valor),
      cerrar_terminar(cerrar)
{
  ...
  connect(ui->twSeleccionar, SIGNAL(doubleClicked(QModelIndex)), this,
          SLOT(aceptar()));
  connect(ui->twSeleccionar, SIGNAL(clicked(const QModelIndex &)), this,
          SLOT(seleccionarObjeto(QModelIndex)));
...
dlgSeleccionarGeneral::~dlgSeleccionarGeneral() { delete ui; }
...
void dlgSeleccionarGeneral::cargarTipo()
{
  ...
  switch (tipo_seleccionado) {
    ...
    case PROVINCIA: {
      ...
      connect(m_provincias, SIGNAL(actualizado()), this,
              SLOT(actualizarObjeto()));
      ui->btVerObjeto->hide();
      break;
    }
    case PERSONA: {
      ...
      connect(m_personas, SIGNAL(actualizado()), this,
              SLOT(actualizarObjeto()));
    ...
    case CAPITULO: {
      ...
      connect(m_capitulos, SIGNAL(actualizado()), this,
              SLOT(actualizarObjeto()));
      ui->btVerObjeto->hide();
      break;
    }
    case DIOCESIS: {
      ...
      connect(m_diocesis, SIGNAL(actualizado()), this,
              SLOT(actualizarObjeto()));
      ui->btVerObjeto->hide();
      break;
    }
   ...
   * m_objeto es igual a m_casas o m_personas. Joder no consigo hacerlo con un
   * if!
   */
  switch (tipo_seleccionado) {
    case PROVINCIA:
...
void dlgSeleccionarGeneral::cargarMenus()
{
  ...
  connect(a_verDiocesisPersona, SIGNAL(triggered(bool)), this,
          SLOT(verDiocesisPersona()));
...
void dlgSeleccionarGeneral::cargarModelo()
{
  ...
  if (tipo_seleccionado == CAPITULO)
    ui->twSeleccionar->setItemDelegateForColumn(
...
{
  if (filtro.length() > 2) {
    m_objeto_proxy->setFilterRegExp(
...
{
  /*
   * ATENCiÖN: esta nota es antigua: ahora emito una casa, pero pierdo lo de la
   * provincia? bueno, se podría hacer con una signal que emita tb el nombre de
   * la provincia...
   *
   * Nota antigua:
   ...
   * tengo que poner lugar/provincia/etc. como int y luego tendría que
   * convertirlo en algún sitio a QString... por eso paso por ahora.
   */
  ...
  QModelIndex idx0 =
      m_objeto_proxy->index(ui->twSeleccionar->currentIndex().row(), 0);
  QModelIndex idx1 =
      m_objeto_proxy->index(ui->twSeleccionar->currentIndex().row(), 1);
  QModelIndex idx2 =
      m_objeto_proxy->index(ui->twSeleccionar->currentIndex().row(), 2);
  QModelIndex idx3 =
      m_objeto_proxy->index(ui->twSeleccionar->currentIndex().row(), 3);
  QModelIndex idx4 =
      m_objeto_proxy->index(ui->twSeleccionar->currentIndex().row(), 4);

  if (!idx0.isValid()) return;

  int id = m_objeto->data(m_objeto_proxy->mapToSource(idx0), Qt::DisplayRole)
               .toInt();
  QString nombre =
      m_objeto->data(m_objeto_proxy->mapToSource(idx1), Qt::DisplayRole)
          .toString();

  // esto eran variables anteriores
  /*
  QString lugar = m_objeto->data(m_objeto_proxy->mapToSource(idx2),
  Qt::DisplayRole).toString(); QString provincia =
  m_objeto->data(m_objeto_proxy->mapToSource(idx3), Qt::DisplayRole).toString();
  QString advocacion = m_objeto->data(m_objeto_proxy->mapToSource(idx4),
...
void dlgSeleccionarGeneral::persona()
{
  ...
  QString nombre =
      m_objeto->data(m_objeto_proxy->mapToSource(idx1), Qt::DisplayRole)
          .toString();
  QString apellidos =
      m_objeto->data(m_objeto_proxy->mapToSource(idx2), Qt::DisplayRole)
          .toString();
  QString origen =
      m_objeto->data(m_objeto_proxy->mapToSource(idx3), Qt::DisplayRole)
          .toString();
...
void dlgSeleccionarGeneral::capitulo()
{
  ...
  QModelIndex idx1 =
      m_objeto_proxy->index(ui->twSeleccionar->currentIndex().row(), 1);
  QModelIndex idx2 = m_objeto_proxy->index(
      ui->twSeleccionar->currentIndex().row(), 2);  // esto es el año
...
void dlgSeleccionarGeneral::verObjeto()
{
  ...
  int id = m_objeto->data(m_objeto_proxy->mapToSource(indice), Qt::DisplayRole)
               .toInt();
  qDebug() << "escogido: " << id;
...
void dlgSeleccionarGeneral::cerrar() { parentWidget()->close(); }
