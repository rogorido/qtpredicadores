...
const QString sql_lord =
    "SELECT DISTINCT details->>'temporal_lord' AS lord FROM "
    "general.dioceses_details ORDER BY lord";
const QString sql_keys =
    "SELECT DISTINCT jsonb_object_keys(details) AS keys FROM "
    "general.dioceses_details ORDER BY keys";
const QString sql_values =
    "SELECT DISTINCT value FROM dioceses_details, jsonb_each_text(details) "
    "WHERE value !~ '[0-9]'";
const QString sql_divisas =
    "SELECT DISTINCT details->>'mensa_currency' as currency FROM "
    "general.dioceses_details "
    "UNION SELECT DISTINCT details->>'tax_currency' as currency FROM "
    "general.dioceses_details "
    "ORDER BY currency";

dlgDiocesisDetalles::dlgDiocesisDetalles(int diocesis_id, QWidget *parent)
    : QWidget(parent), ui(new Ui::dlgDiocesisDetalles), dioc_id(diocesis_id)
{
  ui->setupUi(this);

  mdiarea = MyQmdiArea::Instance(this);

  connect(ui->txtSufraganea, SIGNAL(dobleclick()), this,
          SLOT(AnadirArchiDiocesis()));
...
dlgDiocesisDetalles::~dlgDiocesisDetalles() { delete ui; }
...
void dlgDiocesisDetalles::aceptar()
{
  ...
  if (!rentas_divisa.isEmpty())
    json.insert("mensa_currency", QJsonValue(rentas_divisa));
  if (!tasas_divisa.isEmpty())
    json.insert("tax_currency", QJsonValue(tasas_divisa));
  if (numero_obispos != 0)
    json.insert("number_bishops", QJsonValue(numero_obispos));
...
void dlgDiocesisDetalles::cancelar() { parentWidget()->close(); }

void dlgDiocesisDetalles::AnadirArchiDiocesis()
{
  dlgSeleccionarGeneral *seleccion =
      new dlgSeleccionarGeneral(DIOCESIS, true, this);
  connect(seleccion, SIGNAL(diocesisEscogidaSignal(Diocesis)), this,
          SLOT(recibirArchiDiocesis(Diocesis)));
...
void dlgDiocesisDetalles::introducirDatos(QJsonDocument json_doc)
{
  ...
  query.prepare(
      "INSERT INTO general.dioceses_details(diocese_id, details) "
