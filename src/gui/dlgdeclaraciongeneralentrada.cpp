...
const QString sql_instituciones =
    "SELECT DISTINCT jsonb_array_elements_text(details->'instituciones') AS "
    "instituciones "
    "FROM resolutions_details WHERE details->>'instituciones' IS NOT NULL "
    "ORDER BY instituciones";
const QString sql_cargos =
    "SELECT DISTINCT jsonb_array_elements_text(details->'cargos') AS cargos "
...
dlgDeclaracionGeneralEntrada::dlgDeclaracionGeneralEntrada(QWidget *parent)
    : QWidget(parent), ui(new Ui::dlgDeclaracionGeneralEntrada)
{
  ui->setupUi(this);
  ...
  connect(ui->btEliminarCargos, SIGNAL(clicked(bool)), this,
          SLOT(quitarCargo()));
  connect(ui->btEliminarInstitucion, SIGNAL(clicked(bool)), this,
          SLOT(quitarInstitucion()));
  connect(ui->btEliminarProvincias, SIGNAL(clicked(bool)), this,
...
dlgDeclaracionGeneralEntrada::~dlgDeclaracionGeneralEntrada() { delete ui; }
...
void dlgDeclaracionGeneralEntrada::anadirCargo()
{
  if (!ui->txtCargo->text().isEmpty()) {
    ...
    QListWidgetItem *item =
        new QListWidgetItem(ui->txtCargo->text(), ui->lwCargos);
    ui->txtCargo->setText("");
  }
}
...
void dlgDeclaracionGeneralEntrada::anadirInstitucion()
{
  if (!ui->txtInstitucion->text().isEmpty()) {
    ...
    QListWidgetItem *item =
        new QListWidgetItem(ui->txtInstitucion->text(), ui->lwInstitucion);
...
void dlgDeclaracionGeneralEntrada::recibirProvincia(Provincia provincia)
{
  ...
  QListWidgetItem *item =
      new QListWidgetItem(provincia.getNombre(), ui->lwProvincias);
}

void dlgDeclaracionGeneralEntrada::cerrar() { parentWidget()->close(); }
