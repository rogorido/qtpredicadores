...
const QString sql_tiposcasas =
    "SELECT DISTINCT type_house FROM houses WHERE type_house IS NOT NULL ORDER "
    "BY type_house";

dlgGestionCasas::dlgGestionCasas(QWidget *parent)
    : QWidget(parent), ui(new Ui::dlgGestionCasas)
{
  ui->setupUi(this);
  ...
  connect(ui->twCasas->selectionModel(),
          SIGNAL(currentRowChanged(QModelIndex, QModelIndex)), this,
          SLOT(seleccionarCasa(QModelIndex)));
...
dlgGestionCasas::~dlgGestionCasas() { delete ui; }

void dlgGestionCasas::actualizarFiltro(const QString filtro)
{
  if (filtro.length() > 2) {
    proxy_casas->setFilterRegExp(
...
void dlgGestionCasas::seleccionarCasa(const QModelIndex &idx)
{
  ...
  int id =
      m_casas->data(proxy_casas->mapToSource(indice), Qt::DisplayRole).toInt();
...
void dlgGestionCasas::cerrar() { parentWidget()->close(); }
...
void dlgGestionCasas::generarSQLProvincias()
{
  ...
  for (int var = 0; var < provincias_escogidas.size(); ++var) {
    sql += QString("province_id = ") +
           QString::number(provincias_escogidas.at(var).id) + QString(" OR ");
  }
  // borramos el último OR
  sql.chop(4);
  sql = QString("house_id IN (SELECT DISTINCT house_id FROM houses WHERE ") +
        sql + QString(")");
...
void dlgGestionCasas::on_btModificar_clicked()
{
  ...
  connect(dlgCasaAModificar, SIGNAL(casaIntroducida()), this,
          SLOT(actualizarModeloTrasCasaIntroducida));
  window->show();
}
...
void dlgGestionCasas::recibirProvincia(const Provincia provincia)
{
  ...
  QListWidgetItem *item =
      new QListWidgetItem(nuevaprovincia.elemento, ui->lwProvincias);
  Q_UNUSED(item)
