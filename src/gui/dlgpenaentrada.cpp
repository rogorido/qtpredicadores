...
const QString sql_tipos =
    "SELECT DISTINCT jsonb_array_elements_text(details->'pena_tipos') AS penas "
    "FROM resolutions_details WHERE details->>'pena_tipos' IS NOT NULL "
    "ORDER BY penas";
const QString sql_penatexto =
    "SELECT DISTINCT details->>'pena_texto' AS pena_texto "
    "FROM resolutions_details WHERE details ? 'pena' ORDER BY pena_texto";
const QString sql_penados =
    "SELECT DISTINCT jsonb_array_elements_text(details->'penados') AS penados "
    "FROM resolutions_details WHERE details->>'penados' IS NOT NULL "
    "ORDER BY penados";
const QString sql_motivo =
...
dlgPenaEntrada::dlgPenaEntrada(QWidget *parent)
    : QDialog(parent), ui(new Ui::dlgPenaEntrada)
{
  ui->setupUi(this);

  connect(ui->btCancelar, SIGNAL(clicked(bool)), this, SLOT(cerrar()));
  connect(ui->btOK, SIGNAL(clicked(bool)), this, SLOT(aceptar()));
  connect(ui->btEliminarPenas, SIGNAL(clicked(bool)), this,
          SLOT(quitarPenasTipos()));
  connect(ui->btEliminarPenados, SIGNAL(clicked(bool)), this,
          SLOT(quitarPenados()));
...
dlgPenaEntrada::~dlgPenaEntrada() { delete ui; }
...
void dlgPenaEntrada::aceptar()
{
  ...
  if (ui->wdNotas->haCambiado()) pena.setNotas(ui->wdNotas->getNotas());
...
void dlgPenaEntrada::anadirPenasTipos()
{
  if (!ui->txtTipos->text().isEmpty()) {
    ...
    QListWidgetItem *item =
        new QListWidgetItem(ui->txtTipos->text(), ui->lwTipos);
    ui->txtTipos->setText("");
  }
}
...
void dlgPenaEntrada::anadirPenados()
{
  if (!ui->txtPenados->text().isEmpty()) {
    ...
    QListWidgetItem *item =
        new QListWidgetItem(ui->txtPenados->text(), ui->lwPenados);
...
void dlgPenaEntrada::cerrar() { parentWidget()->close(); }
