...
dlgNuevaObra::dlgNuevaObra(QWidget *parent, int obra)
    : QWidget(parent), ui(new Ui::dlgNuevaObra)
{
  ui->setupUi(this);
  cargarUI();
...
dlgNuevaObra::~dlgNuevaObra() { delete ui; }

void dlgNuevaObra::cargarCompleters()
{
  m_lugaresoriginales = new QSqlQueryModel(this);
  m_lugaresoriginales->setQuery(
      "SELECT DISTINCT place_print_original FROM works.works ORDER BY "
  ...
  m_editores->setQuery(
      "SELECT DISTINCT editor FROM works.works ORDER BY editor");
  ...
  m_idiomas->setQuery(
      "SELECT DISTINCT language_work FROM works.works ORDER BY language_work");
...
{
  if (temasescogidos.size() == 0) return;

  for (int i = 0; i < temasescogidos.size(); ++i) {
    QSqlQuery query;
    query.prepare(
        "INSERT INTO works.works_themes(work_id, theme_id) VALUES (:work, "
        ":tema)");
...
void dlgNuevaObra::cargarUI()
{
  ...
  connect(ui->txtLugar, SIGNAL(dobleclick()), this,
          SLOT(on_btIntroducirLugar_clicked()));
  connect(ui->txtAutor, SIGNAL(dobleclick()), this,
          SLOT(on_btSeleccionarAutor_clicked()));
  connect(ui->btCancelar, SIGNAL(clicked()), this, SLOT(cerrar()));

  cargarCompleters();
}

void dlgNuevaObra::cerrar() { parentWidget()->close(); }
...
void dlgNuevaObra::introducirJson(const int id)
{
  ...
  for (int var = 0; var < totaljson; ++var) {
    ...
    query.prepare(
        "INSERT INTO works_details(work_id, details) VALUES(:workid, :json)");
...
void dlgNuevaObra::cargarObra()
{
  ...
  query.exec(
      QString(
          "SELECT name || ' ' || family_name FROM persons WHERE person_id=%1")
          .arg(obraModificada->getAutor()));
  query.first();
  ...
  // el lugar
  query.exec(QString("SELECT place FROM places WHERE place_id=%1")
                 .arg(obraModificada->getLugar()));
  query.first();
  // atención: es psoible que este campo esté vacío!
  if (query.size() > 0) {
...
void dlgNuevaObra::introducirObraEnBD(Obra *obra, bool modificar)
{
  ...
  if (m_obras->AnadirObra(obra, obra_modificando)) {
    query.exec(QString("DELETE FROM works_themes WHERE work_id = %1")
                   .arg(obra_modificando));
    query.exec(QString("DELETE FROM works_details WHERE work_id = %1")
                   .arg(obra_modificando));
