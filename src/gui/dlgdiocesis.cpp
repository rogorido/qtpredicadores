...
dlgDiocesis::dlgDiocesis(QWidget *parent)
    : QWidget(parent), ui(new Ui::dlgDiocesis)
{
  ui->setupUi(this);
  ...
  // connect(ui->twDiocesis->selectionModel(),
  // SIGNAL(currentRowChanged(QModelIndex,QModelIndex)),
  //       this, SLOT(seleccionarDiocesis(QModelIndex)));
  connect(ui->twDiocesis, SIGNAL(clicked(const QModelIndex &)), this,
          SLOT(seleccionarDiocesis(QModelIndex)));
  ...
  connect(ui->btAnadirDetalles, SIGNAL(clicked()), this,
          SLOT(anadirDetallesDiocesis()));
  connect(ui->btModificarDiocesis, SIGNAL(clicked()), this,
          SLOT(modificarDiocesis()));
}

dlgDiocesis::~dlgDiocesis() { delete ui; }
...
void dlgDiocesis::seleccionarDiocesis(const QModelIndex &idx)
{
  ...
  if (!indice_verdadero.isValid()) return;

  diocesis_seleccionada =
      m_diocesis->data(indice_verdadero, Qt::DisplayRole).toInt();
...
{
  if (filtro.length() > 2) {
    proxy_diocesis->setFilterRegExp(
...
{
  if (diocesis_seleccionada == 0) return;

  dlgNuevaDiocesis *dlgnueva =
      new dlgNuevaDiocesis(diocesis_seleccionada, this);
...
void dlgDiocesis::anadirDetallesDiocesis()
{
  ...
  dlgDiocesisDetalles *dlgnueva =
      new dlgDiocesisDetalles(diocesis_seleccionada, this);
  QMdiSubWindow *window = mdiarea->addSubWindow(dlgnueva);
  window->show();
}
...
void dlgDiocesis::cerrar() { parentWidget()->close(); }
...
QString dlgDiocesis::extraerUrl()
{
  ...
  query.prepare(
      "SELECT other_data->'gcatholic'->>'url' FROM dioceses WHERE diocese_id = "
      ":diocesis");
...
void dlgDiocesis::cargarMenus()
{
  ...
  connect(a_modificarDiocesis, SIGNAL(triggered(bool)), this,
          SLOT(modificarDiocesis()));
...
{
  QModelIndex indice =
      proxy_diocesis->index(ui->twDiocesis->currentIndex().row(), 0);
  if (!indice.isValid()) return;

  int id =
      m_diocesis->data(proxy_diocesis->mapToSource(indice), Qt::DisplayRole)
          .toInt();
  qDebug() << "escogido: " << id;
}
...
{
  QModelIndex indice1 =
      proxy_diocesis->index(ui->twDiocesis->currentIndex().row(), 13);
  QModelIndex indice2 =
      proxy_diocesis->index(ui->twDiocesis->currentIndex().row(), 0);
  if (!indice1.isValid() || !indice2.isValid()) return;
  bool vistos_todos =
      m_diocesis->data(proxy_diocesis->mapToSource(indice1), Qt::DisplayRole)
          .toBool();
  int id =
      m_diocesis->data(proxy_diocesis->mapToSource(indice2), Qt::DisplayRole)
          .toInt();
  ...
  QSqlQuery query;
  query.prepare(
      "UPDATE dioceses SET check_allbishops = :valor WHERE diocese_id = :id");
