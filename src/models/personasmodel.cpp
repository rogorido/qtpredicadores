...
PersonasModel::PersonasModel() : QSqlQueryModel()
{
  // cargamos la query...
  cargarQueryInicial();
...
bool PersonasModel::AnadirPersona(const Persona *persona, int persona_id)
{
   ...
   */
  if (otrosnombres.isEmpty()) otrosnombres = "{}";

  if (persona_id == 0) {
    query.prepare(
        "INSERT INTO general.persons(name, family_name, origin_name, lookedup, "
        "male, wikipedia, viaf, "
        "other_names, wikipedia_link, viaf_link, wikidata_link, type_person, "
        "datebirth, "
        "datedeath, birth_diocesis, look_again, quantity_info, notes) "
        "VALUES (:nombre, :apellidos, :origen, :buscado, :masculino, "
        ":wikipedia, :viaf, "
        ":otrosnombres, :wikipedia_link, :viaf_link, :wikidata_link, "
        ":tipo_persona, :nacimiento, "
        ":muerte, :diocesis, :volveramirar, :cantidad_info, :notas)");
  }
  else {
    query.prepare(
        "UPDATE general.persons SET (name, family_name, origin_name, lookedup, "
        "male, wikipedia, viaf, "
        "other_names, wikipedia_link, viaf_link, wikidata_link, type_person, "
        "datebirth, "
        "datedeath, birth_diocesis, look_again, quantity_info, notes) "
        "= (:nombre, :apellidos, :origen, :buscado, :masculino, :wikipedia, "
        ":viaf, "
        ":otrosnombres, :wikipedia_link, :viaf_link, :wikidata_link, "
        ":tipo_persona, :nacimiento, "
        ":muerte, :diocesis, :volveramirar, :cantidad_info, :notas) "
  ...
  if (persona_id != 0) query.bindValue(":person_id", persona_id);
...
Persona *PersonasModel::devolverPersona(const int id)
{
  ...
  persona->setApellidos(
      query.value(Persona::CamposPersona::FAMILY_NAME).toString());
  ...
  persona->setWikilink(
      query.value(Persona::CamposPersona::WIKIPEDIA_LINK).toString());
  persona->setViaflink(
      query.value(Persona::CamposPersona::VIAF_LINK).toString());
  persona->setWikidata(
      query.value(Persona::CamposPersona::WIKIDATA_LINK).toString());
  persona->setNacimiento(
      query.value(Persona::CamposPersona::DATEBIRTH).toString());
  persona->setMuerte(query.value(Persona::CamposPersona::DATEDEATH).toString());
  persona->setCantidadInfo(
      query.value(Persona::CamposPersona::QUANTITY_INFO).toInt());
  // TODO: Atención falta lo de interesting
  persona->setVolverMirar(
      query.value(Persona::CamposPersona::LOOK_AGAIN).toBool());
  persona->setNotas(query.value(Persona::CamposPersona::NOTES).toString());
  // TODO: falta other-data pq es jsonb
  persona->setOrigen(
      query.value(Persona::CamposPersona::ORIGIN_NAME).toString());
  persona->setDiocesis(
      query.value(Persona::CamposPersona::BIRTH_DIOCESIS).toString());
  persona->setMasculino(query.value(Persona::CamposPersona::MALE).toBool());
  persona->setTipoPersona(
      query.value(Persona::CamposPersona::TYPE_PERSON).toString());
...
QJsonModel *PersonasModel::devolverDetalles(const int id)
{
  ...
  sql =
      QString("SELECT details FROM persons_details WHERE person_id=%1").arg(id);
  query.exec(sql);
