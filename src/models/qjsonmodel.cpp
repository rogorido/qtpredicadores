...
int QJsonModel::columnCount(const QModelIndex &parent) const { return 2; }
...
QVariant QJsonModel::data(const QModelIndex &index, int role) const
{
  ...
  if (role == Qt::DisplayRole) {
    ...
    if (index.column() == 1) {
      if (item->getKey() == "lugar" or item->getKey() == "viaje_origen" or
          item->getKey() == "viaje_destino") {
        valor = sacarLugar(item->getValue().toInt());
        return valor;
      }
      else if (item->getKey() == "casa" || item->getKey() == "casa_origen" ||
               item->getKey() == "casa_destino") {
        valor = sacarCasa(item->getValue().toInt());
        return valor;
      }
      else if (item->getKey() == "provincia" ||
               item->getKey() == "provincia_origen" ||
               item->getKey() == "provincia_destino") {
...
void QJsonModel::anadirJson(QJsonObject json) { lista_jsons.append(json); }

QJsonObject QJsonModel::getJsonObject(int i) { return lista_jsons.at(i); }

int QJsonModel::getSize() { return lista_jsons.size(); }
