...
QVariant AprobacionesTableModel::headerData(int section,
...
QVariant AprobacionesTableModel::data(const QModelIndex &index, int role) const
{
  ...
  if (role == Qt::DisplayRole) {
    ...
    if (aprobacion->getTipoAprobacion() == Aprobacion::TipoAprobacion::PERSONA)
      persona = aprobacion->getPersona().getNombre() + ' ' +
                aprobacion->getPersona().getApellidos();
    else
      casa = aprobacion->getCasa().getNombre();

    if (index.column() == 0) {
      if (aprobacion->getTipoAprobacion() ==
          Aprobacion::TipoAprobacion::PERSONA)
