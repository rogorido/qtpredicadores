...
ObrasModel::ObrasModel() : QSqlTableModel()
{
...
bool ObrasModel::AnadirObra(const Obra *obra, int obra_id)
{
  ...
  if (obra_id == 0) {
    query.prepare(
        "INSERT INTO works.works(title, language_work, author_id, type_work, "
        "format, "
        ...
        "contents, interesting, dubious, expurgatable, reliability, "
        "page_quetif, notes) "
        " VALUES (:titulo, :idioma, :autor_id, :tipo, :formato, "
        ":tomos, :numero_pags, :impreso, :talvez_impreso, :manuscrito, "
        ":volveramirar, :investigar, "
        ":lugar_impresion_original, :lugar_impresion_id, "
        ":fecha_impresion, :editor, :referencias, :tituloreducido, "
        ":traduccion, "
        ":contenido, :interesante, :dudoso, :expurgable, :fiabilidad, "
        ":pagequetif, :notas)");
  }
  else {
    query.prepare(
        "UPDATE works.works SET(title, language_work, author_id, type_work, "
        "format, "
        "volumes, number_pages, printed, maybe_printed, manuscrit, look_again, "
        "investigate, "
        "place_print_original, place_print_id, "
  ...
  if (obra_id != 0) query.bindValue(":work_id", obra_id);
...
Obra *ObrasModel::devolverObra(const int id)
{
  ...
  obra->setLugarOriginal(
      query.value(Obra::CamposObra::PLACE_PRINT_ORIGINAL).toString());
  ...
  obra->setTituloReducido(
      query.value(Obra::CamposObra::REDUCED_TITLE).toBool());
...
QVector<int> ObrasModel::materiasObra(const int obra_id)
{
  ...
  QString sql =
      QString("SELECT * FROM works_themes WHERE work_id = %1").arg(obra_id);
