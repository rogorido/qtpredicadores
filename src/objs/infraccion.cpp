...
void Infraccion::setInfractores(const QStringList i) { infractores = i; }
...
QJsonObject Infraccion::getInfraccionJson()
{
  ...
  if (!tipo.isEmpty()) json.insert("infraccion_tipo", tipo);

  if (!infraccion.isEmpty()) json.insert("infraccion_asunto", infraccion);

  if (!infractores.isEmpty())
    json.insert("infraccion_infractores",
                QJsonArray::fromStringList(infractores));
